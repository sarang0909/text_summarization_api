"""TO BE IMPLEMENTED - A main script to run api.
...
async def get_model_output(input_data: InputData):
    ...
    Args:
        ...
        model(str):model type "Custom Model" or "Reused Model"
    Returns:
        output(str) : text summary

    """

    try:
        predictor = get_predictor(input_data.model)
        output = predictor.get_model_output(input_data.text)
